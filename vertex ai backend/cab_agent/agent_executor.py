"""Cab Agent Executor for A2A integration."""
...
class CabADKAgentExecutor(AgentExecutor):
    """ADK Agent Executor for cab A2A integration."""
    ...
    def __init__(self, agent: Agent, agent_card: AgentCard, runner: Runner):
        ...
        Args:
            agent: The cab ADK agent instance
            agent_card: Agent card for A2A service registration
            runner: Pre-configured ADK Runner instance
        """
        logger.info(f"Initializing CabADKAgentExecutor for agent: {agent.name}")
        self.agent = agent
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        """Execute the cab agent's logic for a given request context.
    ...
    async def _send_response(
        ...
        logger.info(f"Sending cab search response for task {context.task_id}")
    ...
    async def _handle_error(
        ...
        logger.error(
            f"Error executing cab search in {self.agent.name}: {str(error)}",
            exc_info=True,
        )
        error_message_text = f"Error searching cab workspace: {str(error)}"
        await event_queue.enqueue_event(
            new_agent_text_message(
    ...
    async def cancel(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        logger.info(
            f"Cancelling cab search task: {task_id} for agent {self.agent.name}"
        )
        ...
        logger.info(f"Sent cancel event for cab task: {task_id}")