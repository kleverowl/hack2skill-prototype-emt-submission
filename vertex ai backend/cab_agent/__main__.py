...
from cab_agent.agent import agent
...
class CabAgentExecutor:
    def __init__(self):
        self.broker = MessageBroker()
        self.agent_name = "cab_agent"
                ...
                try:
                    ...
                    if 'results' in llm_response_json:
                        ...
                        if results and len(results) > 0:
                            cab = results[0]
                            friendly_response = f"🚕 **{cab.get('name', 'Cab Service')}**\n\n"
                            friendly_response += f"🚗 Type: {cab.get('cab_type', 'N/A')}\n"
                            friendly_response += f"💰 Price: {cab.get('currency', '$')}{cab.get('price', 'N/A')}\n"
                            if cab.get('phone'):
                                friendly_response += f"📞 Contact: {cab.get('phone')}\n"
                            if cab.get('justification'):
                                friendly_response += f"\n💡 Note: {cab.get('justification')}"
