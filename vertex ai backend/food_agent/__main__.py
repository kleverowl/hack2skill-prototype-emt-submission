...
class FoodAgentExecutor:
    def __init__(self):
        self.broker = MessageBroker()
        self.agent_name = "food_agent"
                ...
                try:
                    ...
                    if 'results' in llm_response_json:
                        ...
                        if results and len(results) > 0:
                            ...
                            friendly_response = f"🍽️ **{restaurant.get('name', 'Restaurant')}**\n\n"
                            friendly_response += f"🍴 Cuisine: {restaurant.get('cuisine_type', 'N/A')}\n"
                            friendly_response += f"📍 Address: {restaurant.get('address', 'N/A')}\n"
                            friendly_response += f"💰 Price Range: {restaurant.get('price_range', 'N/A')}\n"
                            friendly_response += f"⭐ Rating: {restaurant.get('rating', 'N/A')}\n"
                            if restaurant.get('justification'):
                                friendly_response += f"\n💡 Note: {restaurant.get('justification')}"
