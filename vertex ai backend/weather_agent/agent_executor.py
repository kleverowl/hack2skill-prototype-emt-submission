"""Weather Agent Executor for A2A integration."""
...
class WeatherADKAgentExecutor(AgentExecutor):
    """ADK Agent Executor for weather A2A integration."""
    ...
    def __init__(self, agent: Agent, agent_card: AgentCard, runner: Runner):
        ...
        Args:
            agent: The weather ADK agent instance
            agent_card: Agent card for A2A service registration
            runner: Pre-configured ADK Runner instance
        """
        logger.info(f"Initializing WeatherADKAgentExecutor for agent: {agent.name}")
        self.agent = agent
    ...
    def _prepare_input(self, context: RequestContext) -> str:
        ...
        if not user_input:
            ...
            user_input = "What is the weather in Nashik?"
    ...
    async def _run_agent_and_get_response(
        ...
        final_message_text = "(Could not retrieve weather information)"
    ...
    async def _send_response(
        ...
        logger.info(f"Sending weather search response for task {context.task_id}")
    ...
    async def _handle_error(
        ...
        logger.error(
            f"Error getting weather in {self.agent.name}: {str(error)}",
            exc_info=True,
        )
        error_message_text = f"Error searching weather workspace: {str(error)}"
        await event_queue.enqueue_event(
            new_agent_text_message(
    ...
    async def cancel(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        logger.info(
            f"Cancelling weather task: {task_id} for agent {self.agent.name}"
        )
        ...
        logger.info(f"Sent cancel event for weather task: {task_id}")