"""Itinerary Agent Executor for A2A integration (Orchestrator)."""
...
class MainADKAgentExecutor(AgentExecutor):
    """ADK Agent Executor for itinerary A2A integration."""
    ...
    def __init__(self, agent: Agent, agent_card: AgentCard, runner: Runner):
        ...
        Args:
            agent: The itinerary ADK agent instance (orchestrator)
            agent_card: Agent card for A2A service registration
            runner: Pre-configured ADK Runner instance
        """
        logger.info(f"Initializing ItineraryADKAgentExecutor for agent: {agent.name}")
        self.agent = agent
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        """Execute the itinerary agent's logic for a given request context.
    ...
    def _prepare_input(self, context: RequestContext) -> str:
        ...
        if not user_input:
            logger.warning(
                f"No user input found for {self.agent.name}; using default query."
            )
            user_input = "Plan a 2-day trip to Mumbai from Nashik"

        logger.info(f"{self.agent.name} processing query: '{user_input}'")
        return user_input
    ...
    async def _run_agent_and_get_response(
        ...
        final_message_text = "(Sorry, I could not create an itinerary at this time.)"
    ...
    async def _handle_error(
        ...
        logger.error(
            f"Error creating itinerary in {self.agent.name}: {str(error)}",
            exc_info=True,
        )
        error_message_text = f"Sorry, an error occurred while planning your trip: {str(error)}"
        await event_queue.enqueue_event(
            new_agent_text_message(
    ...
    async def cancel(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        logger.info(
            f"Cancelling itinerary task: {task_id} for agent {self.agent.name}"
        )
        ...
        logger.info(f"Sent cancel event for itinerary task: {task_id}")