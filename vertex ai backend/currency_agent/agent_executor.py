...
class CurrencyADKAgentExecutor(AgentExecutor):
    ...
    def __init__(self, agent: Agent, agent_card: AgentCard, runner: Runner):
        ...
        """
        logger.info(f"Initializing CurrencyADKAgentExecutor for agent: {agent.name}")
        self.agent = agent
    ...
    async def _send_response(
        ...
        logger.info(f"Sending currency search response for task {context.task_id}")
    ...
    async def _handle_error(
        ...
        logger.error(
            f"Error executing currency search in {self.agent.name}: {str(error)}",
            exc_info=True,
        )
        error_message_text = f"Error searching currency workspace: {str(error)}"
        await event_queue.enqueue_event(
            new_agent_text_message(
    ...
    async def cancel(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        logger.info(
            f"Cancelling currency search task: {task_id} for agent {self.agent.name}"
        )
