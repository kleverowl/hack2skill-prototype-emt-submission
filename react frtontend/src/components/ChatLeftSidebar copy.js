...
const createNewItinerary = async () => {
  ...
  const db = getDatabase();
  const itinerariesRef = ref(db, `users/user_id/${currentUser.uid}/itineraries`);
    ...
    return (
        <div className="chat-leftsidebar me-lg-1 ms-lg-0">
            <div className="tab-content">
                ...
                <div className={`tab-pane fade ${activeTab === 'chat' ? 'show active' : ''}`} id="pills-chat" role="tabpanel" aria-labelledby="pills-chat-tab">
                    {/* Start chats content */}
                    <div>
                        <div className="px-4 pt-4">
<div className="d-flex flex-column flex-md-row justify-content-between align-items-center px-4 pt-4 mb-3">
  <h4 className="mb-3 mb-md-0 fw-bold text-truncate">
    Ease My Trip Holiday
  </h4>
  <button
...
<div className="search-box chat-search-box mb-3">
  ...
  >
    ...
    />
                        ...
                        <div className="">
                            <h5 className="mb-3 px-3 font-size-16">Recent Itineraries</h5>
                            ...
                            <div className="chat-message-list px-2" data-simplebar>
                                ...
                                <ul className="list-unstyled chat-list chat-user-list">
                                    {Object.keys(itineraries).map((itineraryId, index) => {
                                        ...
                                        const lastMessage = getLastMessage(itinerary.messages);
                                        const tripName = itinerary.state?.itinerary?.trip_name || `Itinerary ${index + 1}`;
                                        const isActive = itineraryId === selectedItineraryId;

                                        return (
                                            <li key={itineraryId} className={isActive ? 'active' : ''}>
                                                <a href="#" onClick={(e) => { e.preventDefault(); onItinerarySelect(itineraryId); }}>
                                                    <div className="d-flex">                            
                                                        <div className="chat-user-img online align-self-center me-3 ms-0">
                                                            <div className="avatar-xs">
                                                                <span className="avatar-title rounded-circle bg-primary text-white" style={{backgroundColor: '#0980B5'}}>
                                                        ...
                                                        <div className="flex-grow-1 overflow-hidden">
                                                            <h5 className="text-truncate font-size-15 mb-1">{tripName}</h5>
                                                            <p className="chat-user-message text-truncate mb-0">{lastMessage.message}</p>
                                                        </div>
                                                        <div className="font-size-11">{new Date(lastMessage.timestamp).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</div>
                                                    </div>
                                                </a>
                                            </li>
                                        )
