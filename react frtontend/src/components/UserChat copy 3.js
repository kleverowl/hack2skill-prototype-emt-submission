...
const UserChat = ({ selectedItineraryId }) => {
    ...
    useEffect(() => {
        ...
        const messagesRef = ref(database, `users/user_id/${currentUser.uid}/itineraries/${selectedItineraryId}/messages`);
    ...
    const sendMessage = async () => {
        if (newMessage.trim() === "" || !user || !selectedItineraryId) {
            console.log("Cannot send message: missing input, user, or itinerary.", user, newMessage, selectedItineraryId);
            return;
        }

        try {
            const messagesRef = ref(database, `users/user_id/${user.uid}/itineraries/${selectedItineraryId}/messages`);
            ...
            setTimeout(async () => {
                ...
                const botMessageId = botMessageRef.key;
                const botMessage = {
                    id: botMessageId,
                    sender: "model",
                    message_type: "text",
    ...
    const TextMessage = ({ message }) => (
        <div className="ctext-wrap-content">
            ...
            <p className="chat-time mb-0">
                ...
                <span className="align-middle">{new Date(message.timestamp).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
            </p>
        </div>
    );
        ...
        <div className="user-chat w-100 overflow-hidden d-flex flex-column vh-100">
            <div className="d-lg-flex">
                <div className="w-100 overflow-hidden position-relative">
                    ...
                    <div className="chat-conversation p-3 flex-grow-1" style={{ overflowY: 'auto' }}>
                        <ul className="list-unstyled mb-0">
                            {messages.map((message, index) => (
                                <li key={index} className={message.sender === 'user' ? 'right' : ''}>
